"""检修任务接口：维护检修任务，覆盖开始任务、提交验收、确认完成与批量转派等动作。"""
...
from app.schemas import ActionResult, BatchActionResult, BatchReassignPayload, EntryPayload, PageResult
...
def summary() -> dict[str, int]:
    """列表上方的统计卡片：待开始、检修中、遗留问题合计与遗留超标任务数。"""
...
@router.post("/batch-reassign", response_model=BatchActionResult)
def batch_reassign(payload: BatchReassignPayload) -> BatchActionResult:
    """多选任务按同一批检修人员转派：逐条校验、逐条给结果，失败的任务留在原状态。"""
    return BatchActionResult(**service.batch_reassign(payload.ids, payload.assignee))
